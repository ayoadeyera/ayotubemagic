...
const IdeasManagerView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (raw) {
      ...
      // Migration: Ensure all items have the required manager fields
  ...
  return (
    <div className="space-y-8 pb-24">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          <h2 className="text-3xl font-bold text-slate-100">Content Pipeline</h2>
          <p className="text-slate-400 mt-1">Manage, categorize, and prioritize your generated assets.</p>
        </div>
        
        <div className="flex gap-4">
          <div className="bg-slate-900/50 border border-slate-800 px-6 py-3 rounded-2xl flex items-center gap-6 shadow-xl">
            <div className="text-center">
              <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Assets</span>
              <span className="text-xl font-bold text-slate-200">{stats.total}</span>
            </div>
            <div className="w-px h-8 bg-slate-800"></div>
            <div className="text-center">
              <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Follow-up</span>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-1.5 bg-slate-900 border border-slate-800 p-1.5 rounded-2xl">
          {['All', ...CATEGORIES].map(cat => (
            <button
              key={cat}
              onClick={() => setSelectedCategory(cat)}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                selectedCategory === cat ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800'
              }`}
            >
              {cat}
            </button>
      ...
      {filteredItems.length > 0 ? (
          ...
          {filteredItems.map((item) => (
            <div 
              ...
              className={`group bg-slate-900 border transition-all rounded-3xl flex flex-col overflow-hidden ${
                item.followUp ? 'border-amber-500/40 bg-amber-500/5' : 'border-slate-800 hover:border-slate-700'
              }`}
            >
              <div className="p-6 space-y-4 flex-1">
                <div className="flex justify-between items-start">
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${getCategoryStyles(item.category)}`}>
                    {item.category}
                  </span>
                  <button 
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-100 leading-tight mb-2 group-hover:text-indigo-400 transition-colors">
                    {item.title}
                  </h3>
                ...
                {editingId === item.id ? (
                  ...
                  >
                    {item.notes ? (
                      <p className="text-xs text-slate-400 italic line-clamp-3 bg-slate-950/30 p-3 rounded-xl border border-slate-800/50">
                        "{item.notes}"
                      </p>
                    ) : (
                      <div className="text-[10px] text-slate-600 border border-dashed border-slate-800 p-3 rounded-xl hover:border-slate-500 transition-all text-center">
        ...
        <div className="flex flex-col items-center justify-center py-32 text-center space-y-4">
          <div className="w-20 h-20 bg-slate-900 rounded-3xl flex items-center justify-center text-slate-700 border border-slate-800 shadow-inner">
          ...
          <div className="max-w-xs">
            ...
            <p className="text-slate-500 text-sm mt-1">Try searching for something else or create a new script to fill your pipeline.</p>
          </div>
        </div>
      )}
    </div>
