...
export class GeminiService {
  private getAI() {
    ...
    if (!apiKey) {
      throw new Error("API Key is missing. Please ensure the API_KEY environment variable is set in Vercel or your local environment.");
    }
  ...
  async generateIdeas(niche: string): Promise<any[]> {
    ...
    try {
      ...
      throw new Error(`Ideas Generation Failed: ${error.message || 'Unknown Error'}`);
    }
  }
  ...
  async researchKeywords(seed: string): Promise<KeywordResult[]> {
    ...
    try {
      ...
      throw new Error(`Keyword Research Failed: ${error.message || 'Unknown Error'}`);
    }
  }
  ...
  async researchNiches(category: string): Promise<NicheResult[]> {
    ...
    try {
      ...
      throw new Error(`Niche Research Failed: ${error.message || 'Unknown Error'}`);
    }
  }
